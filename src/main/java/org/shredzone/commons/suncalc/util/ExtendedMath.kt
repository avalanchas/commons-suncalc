...
object ExtendedMath {
     ...
     * PI * 2
     */
    const val PI2 = Math.PI * 2.0

    /**
     * Arc-Seconds per Radian.
     */
    val ARCS = Math.toDegrees(3600.0)
     ...
     */
    val REFRACTION_AT_HORIZON = Math.PI / (tan(Math.toRadians(7.31 / 4.4)) * 10800.0)
    ...
    fun isZero(d: Double): Boolean {
        ...
        return !java.lang.Double.isNaN(d) && sign(d).roundToLong() == 0L
    }
    ...
    fun equatorialToHorizontal(tau: Double, dec: Double, dist: Double, lat: Double): Vector {
        return Matrix.rotateY(Math.PI / 2.0 - lat)
    ...
    fun equatorialToEcliptical(t: JulianDate): Matrix {
        ...
        val eps =
            Math.toRadians(23.43929111 - (46.8150 + (0.00059 - 0.001813 * jc) * jc) * jc / 3600.0)
     ...
     */
    fun readjustMax(time: Double, frame: Double, depth: Int, f: Function<Double?, Double>): Double {
        val left = time - frame
        val right = time + frame
        val leftY = f.apply(left)
        val rightY = f.apply(right)
     ...
     */
    fun readjustMin(time: Double, frame: Double, depth: Int, f: Function<Double?, Double>): Double {
        val left = time - frame
        val right = time + frame
        val leftY = f.apply(left)
        val rightY = f.apply(right)
