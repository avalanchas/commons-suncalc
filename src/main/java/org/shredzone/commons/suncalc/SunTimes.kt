...
class SunTimes private constructor(
     ...
     */
    interface Parameters : GenericParameter<Parameters?>, LocationParameter<Parameters?>,
    ...
    private class SunTimesBuilder : BaseBuilder<Parameters?>(), Parameters {
        ...
        private var limit = Duration.ofDays(365L)
        ...
        override fun limit(duration: Duration?): Parameters {
            require(!(duration == null || duration.isNegative)) { "duration must be positive" }
            limit = duration
            return this
        }
        ...
        override fun execute(): SunTimes {
            ...
            var ye: Double
            var hour = 0
            val limitHours = limit.toMillis() / (60 * 60 * 1000.0)
            val maxHours = Math.ceil(limitHours).toInt()
            ...
            return SunTimes(
                if (rise != null) jd.atHour(rise).dateTime else null,
                if (set != null) jd.atHour(set).dateTime else null,
                if (noon != null) jd.atHour(noon).dateTime else null,
                if (nadir != null) jd.atHour(nadir).dateTime else null,
                alwaysUp,
                alwaysDown
            )
        }
